...
export default function Component() {
    ...
    return (
        <MainContainer>
            <TextHeader>아리아리의 회원이 되어주셔서 감사드립니다.</TextHeader>
            <LoginContainer isSelected={selectedBox !== null} selected={isWho}>
                <TopContainer>
                    <Box isSelected={isWho === 1} onClick={() => handleClick(1)}>남한 주민</Box>
                    <Box isSelected={isWho === 2} onClick={() => handleClick(2)}>북한 주민</Box>
                    <Box isSelected={isWho === 3} onClick={() => handleClick(3)}>남한 기업</Box> {/* 마지막 세로선 제거 */}
                </TopContainer>
                <Section>
                    <Login_row>
                        <Image src={'icon_id.svg'} />
                        <Input type="text" placeholder="아이디를 입력하세요." />
                        <StyledButton onClick={handleCheckID}>
                            아이디 중복 검사
                        </StyledButton>
                    </Login_row>
                </Section>
                ...
                <Section style={{ borderBottom: 'none' }}>
                    <Login_row>
                        <Image src={'icon_house.svg'} />
                        <Input type="text" placeholder="주소를 입력하세요." />
                    </Login_row>
                </Section> {/* 마지막 가로선 제거 */}
            </LoginContainer>
            <LoginButton onClick={handleSignup}>회원가입 완료</LoginButton>
        </MainContainer>
    );
};
