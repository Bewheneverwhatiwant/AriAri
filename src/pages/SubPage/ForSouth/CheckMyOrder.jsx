...
export default function Component() {
    ...
    const [orders, setOrders] = useState([
        { name: '직접 재배한 단맛 나는 고려 인삼', address: '(내 주소) 경기도 용인시 처인구 모현읍 외대로 81', title: '10점' },
        { name: '벌레 먹은 곳 없는 붉은 사과', address: '(내 주소) 경기도 용인시 처인구 모현읍 외대로 81', title: '15점' },
        { name: '알이 굵고 아삭아삭한 배추', address: '(내 주소) 경기도 용인시 처인구 모현읍 외대로 81', title: '8점' }
    ]);
    ...
    const handleRejectOrderClick = index => {
        if (window.confirm('이 판매자를 신고하시겠습니까?')) {
            const newStatus = [...deliveryStatus];
            newStatus[index] = '판매자 신고';
            setDeliveryStatus(newStatus);
        }
    };
            ...
            {orders.map((order, index) => (
                <OrderContainer key={index}>
                    <ContainerColumn>
                        ...
                        {showButtons[index] && (
                            <ContainerRow>
                                <Mini onClick={() => console.log('상품 등록 취소')}>주문 취소</Mini>
                                <RedMini onClick={() => handleRejectOrderClick(index)}>판매자 신고하기</RedMini>
                            </ContainerRow>
                        )}
                    </ContainerColumn>
                </OrderContainer>
